  ...
  return (
    <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
      ...
      <MotionWrapper
        ...
        className="relative overflow-hidden rounded-[2.5rem] border bg-muted/20 shadow-xl"
      >
        <Carousel setApi={setCarouselApi} className="w-full h-full group/carousel">
          <CarouselContent>
            {slides.map((slide, index) => (
              <CarouselItem key={index} className="relative aspect-square w-full">
                ...
                {/* Premium Glassmorphism Indicator Overlay */}
                <div className="absolute top-5 left-9 z-20 rounded-full border border-white/10 bg-black/60 px-4 py-1.5 text-xs font-semibold text-white/90 backdrop-blur-md shadow-md select-none">
                  {slide.label}
                </div>
              </CarouselItem>
            ))}
          ...
          {slides.length > 1 && (
            <>
              <CarouselPrevious className="absolute left-4 top-1/2 -translate-y-1/2 z-30 opacity-0 group-hover/carousel:opacity-100 transition-opacity bg-background/80 hover:bg-background border-border" />
              <CarouselNext className="absolute right-4 top-1/2 -translate-y-1/2 z-30 opacity-0 group-hover/carousel:opacity-100 transition-opacity bg-background/80 hover:bg-background border-border" />
            </>
          )}
        </Carousel>
        ...
        {slides.length > 1 && (
          <div className="absolute bottom-5 right-5 z-20 flex gap-1.5 bg-black/45 p-2 rounded-full backdrop-blur-sm">
            {slides.map((_, index) => (
              <button
                key={index}
